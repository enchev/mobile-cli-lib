...
import iOSProxyServices = require("../mobile/ios/ios-proxy-services");
import usbLivesyncServiceBaseLib = require("../services/usb-livesync-service-base");
...
export class LiveSyncService extends usbLivesyncServiceBaseLib.UsbLiveSyncServiceBase /*implements ILiveSyncService */ {
	...
	private livesyncOnDevices(deviceIdentifiers: string[], files: string[], appIdentifier: string, projectType: string, forCompanionApp?: boolean): IFuture<string>[] {
		_.each(deviceIdentifiers, )
	}

	private livesyncOnDevice(): IFuture<string> {
		return (() => {

		}).future<string>()();
	}

	public livesync(projectDir: string, appIdentifier: string, platform?: string): IFuture<void> {
		return (() => {
			...
			let notInstalledAppOnDeviceAction = (device: Mobile.IDevice): IFuture<void> => {
				return (() => {
					this.$errors.failWithoutHelp(`Unable to find application with identifier ${appIdentifier} on device ${device.deviceInfo.identifier}.`);
				}).future<void>()();
			};
			...
			console.log("finished livesyncing");
		}).future<void>()();
	}
}
